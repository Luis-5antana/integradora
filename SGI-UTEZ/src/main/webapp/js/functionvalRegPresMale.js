...
//obtener
        ...
        */
        if (opcion == true) {
            regPresMale(nombre,apo_pto,apo_mto,email,matri,fecha,disp,estatus,dispo_estatus,tel);

        }
    }
    function regPresMale(nom,apo_pto,apo_mto,correo,matricula,fecha,id_disp,estatus,est_disp,tel) {
        var url = "ServletPrestMale?nom="+nom+"&apo_pto="+apo_pto+"&apo_mto="+apo_mto+"&email="+correo+"&matri="+matricula+"&fecha="+fecha+"&disp="+id_disp+"&estatus="+estatus+"&disp_estatus"+est_disp+"&tel="+tel;
        $.ajax({
            type: "POST",
            url: url,
            async: true,
